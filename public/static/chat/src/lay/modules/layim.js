...
layui.define(['layer', 'laytpl', 'upload'], function(exports){
    ...
    var removeList = function(data){
        ...
        if(cache[data.type]){
            ...
            }else if(data.type === 'friend_group'){
                console.log(455)
                layui.each(cache.friend, function(index, item){
                    if(data.id == item.id){
                        var li = listElem.find('>li');
                        var list = li.find('.layui-layim-list>li');
                        li.find('.layui-layim-list>li').remove();
                        cache.friend[index].list = []; //从cache的friend里面也删除掉好友
                        li.find('.layim-count').html(cache.friend[index].list.length); //刷新好友数量
                        //如果一个好友都没了
                        if(cache.friend[index].list.length === 0){
                            li.find('.layui-layim-list').html('<li class="layim-null">该分组下已无好友了</li>');
                        }
                        return true;
                    }
